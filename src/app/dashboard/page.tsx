...
export default function DashboardPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    // 🔥 BUSCA TODAS AS TRANSAÇÕES DESSE USUÁRIO (SEM FILTRO DE MÊS)
    let query = supabase
      .from('transactions')
      .select('*')
      .eq('user_id', user.id);
    ...
    // 🔹 separa passado e futuro com base em HOJE (não no mês)
    const past = data.filter((tx) => normalizeDate(tx) <= today);
    const future = data.filter((tx) => normalizeDate(tx) > today);
    ...
    setLoading(false);
  }, [user, selectedProfile, profiles, supabase]);

  useEffect(() => {
    fetchData();
  }, [fetchData]);

  // 🔄 realtime: quando tabela mudar, recalcula
